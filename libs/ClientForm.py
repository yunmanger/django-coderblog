...
def newquote(f):
    def nf(context):
        ...
        return f(context)
    return nf
urlquote = newquote(urllib2.quote)
    ...
    if not doseq:
        ...
        for k, v in query:
            ...
            if type(v) == types.StringType:
                ...
                v = urlquote(v.encode("ASCII","replace"))
                l.append(k + '=' + v)
            else:
                try:
                    ...
                    # not a sequence
                    v = urlquote(unicode(v))
                    l.append(k + '=' + v)
                else:
                    # loop over the sequence
                    for elt in v:
                        l.append(k + '=' + urlquote(unicode(elt)))
    return '&'.join(l)
...
def choose_boundary():
    ...
    nonce = "".join([unicode(random.randint(0, sys.maxint-1)) for i in 0,1,2])
    return "-"*27 + nonce
...
class IsindexControl(ScalarControl):
    ...
    url = urlparse.urljoin(page_uri, "?"+urlquote("my isindex value"))
    ...
    def _click(self, form, coord, return_type, request_class=urllib2.Request):
        ...
        parts = rest + (urlquote(self.value), None)
...
class ListControl(Control):
    ...
    def __str__(self):
        ...
        display = [unicode(o) for o in self.items]
...
class ImageControl(SubmitControl):
    ...
    def _totally_ordered_pairs(self):
        ...
        pairs = [
            (self._index, "%s.x" % name, unicode(clicked[0])),
            (self._index+1, "%s.y" % name, unicode(clicked[1])),
            ]
        value = self._value
        if value:
...
# aliases, just to make unicode(control) and unicode(form) clearer
